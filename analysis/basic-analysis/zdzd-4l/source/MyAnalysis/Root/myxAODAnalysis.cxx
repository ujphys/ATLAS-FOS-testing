...
//    ANA_CHECK(event->writeTo(file));
    ...
    ANA_CHECK (book (TH1D("h_m_H", "h_m_H", 100, 100, 1100)));
    ...
    ANA_CHECK (book (TH1D("h_pT_2l_leading", "h_pT_2l_leading", 100, 0, 500))); //pT
    ANA_CHECK (book (TH1D("h_pT_2l_subleading", "h_pT_2l_subleading", 100, 0, 500)));
    ANA_CHECK (book (TH1D("h_pT_4l", "h_pT_4l", 100, 0, 500)));
    ANA_CHECK (book (TH1D("h_eta_l1l2","h_eta_l1l2", 100, -4.5, 4.5))); //eta
    ANA_CHECK (book (TH1D("h_eta_l3l4","h_eta_l3l4", 100, -4.5, 4.5)));
    ANA_CHECK (book (TH1D("h_eta_4l","h_eta_4l", 100, -4.5, 4.5)));
    ...
    ANA_CHECK (book (TH1D("h_deltaR_l2l3","h_deltaR_l2l3", 100, 0, 8)));
    ANA_CHECK (book (TH1D("h_deltaR_l3l4","h_deltaR_l3l4", 100, 0, 8)));
    //MET
    ANA_CHECK (book (TH1F("h_missingET","h_missingET",100,0,1000)));
    ANA_CHECK (book (TH1D("h_missingET_NonInt","h_missingET_NonInt",100,0,800)));
    ...
    for( ; truth_itr != truth_end ; ++truth_itr){
        ...
        if(truth->pdgId()==25){
            ...
            if (truth->hasDecayVtx()){
                ...
                const xAOD::TruthParticle* childZd1 = decayVtx->outgoingParticle(0);
                const xAOD::TruthParticle* childZd2 = decayVtx->outgoingParticle(1);
                //Check the Higgs has decayed to dark Higgs
                if ( (childZd1->absPdgId() == 32) && (childZd2->absPdgId() == 32)){
                    //ANA_MSG_INFO("Dark photons found, children of the Higgs");
                    //Once we know the Higgs decays to two Zd, we store its kinematics
                    pT_H = truth -> pt();
                    eta_H = truth -> eta();
                    phi_H = truth -> phi();
                    m_H = truth -> m();
                    //Check dark photons have decay vertices
                    if (childZd1->hasDecayVtx() && childZd2->hasDecayVtx()){ 
                        //ANA_MSG_INFO("Dark photon found, child of the Higgs");
                        const xAOD::TruthVertex* Zd1_decayVtx = childZd1->decayVtx();
                        const xAOD::TruthVertex* Zd2_decayVtx = childZd2->decayVtx();
                        // ANA_MSG_INFO("Two Zd found");
                        n_Zd++;

                        //------------Label the 2nd gen children (l 1-4)------------
                        ...
                        //Once we know the Zd decay, we store its kinematics
                        pT_Zd1 = childZd1 -> pt(); pT_Zd2 = childZd2 -> pt();
        ...
        int bitmask = pow(2,3)*l1_bool + pow(2,2)*l3_bool;

        //ANA_MSG_INFO(l1_bool << l2_bool << l3_bool << l4_bool);
        //check 44 case
        ...
        hist ("h_m_Zd1") -> Fill(m_Zd1/1000); hist ("h_m_Zd2") -> Fill(m_Zd2/1000);
        //Electrons          
