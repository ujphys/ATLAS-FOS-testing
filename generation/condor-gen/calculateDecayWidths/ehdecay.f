C         Last modification on January 30 2014 by M.S.
C         Last modification on March 12 2014 by MMM
C ==================================================================
C ================= PROGRAM eHDECAY: COMMENTS ======================
C ==================================================================
C This program is based on the original program HDECAY 5.10, with 
C some modifications from HDECAY 6.10 implemented, that are relevant 
C here.  
C Changes with respect to the original version
C are indicated by 'MG and MMM changed'
C The program eHDECAY allows for the calculation of the Higgs
C boson branching ratios and total decay widths based on an 
C effective Lagrangian. At present the user can choose among the
C following parametrisations:
C chiral Lagrangian
C SILH - strongly interacting light Higgs
C MCHM4, MCHM5 - minimal composite Higgs models with fermions in the
C                fundamental (4) or spinorial (5) representation
C
C Details on the models and the implementation within the program
C can be found in:
C
C Effective Lagrangians for a light Higgs-like scalar
C R. Contino, M. Ghezzi, C. Grojean, M. Muhlleitner and M. Spira
C JHEP 1307 (2013) 035 [1303.3876 [hep-ph]]
C
C When using this program, the following publications should be 
C cited:
C
C Effective Lagrangians for a light Higgs-like scalar
C R. Contino, M. Ghezzi, C. Grojean, M. Muhlleitner and M. Spira
C JHEP 1307 (2013) 035 [1303.3876 [hep-ph]]
C
C A. Djouadi, J. Kalinowski and M. Spira, Comput. Phys. 
C Commun. 108 (1998) 56 [hepph/9704448]
C
C A. Djouadi, M. M. Muhlleitner and M. Spira, Acta Phys. Polon. 
C B38 (2007) 635 [hep- ph/0609292].
C ==================================================================
...
C          kalino@fuw.edu.pl
C          margarete.muehlleitner@kit.edu
C          Michael.Spira@psi.ch
C
C
C ================ IT USES AS INPUT PARAMETERS:
C
C MG and MMM changed
C   SLHAIN: =0: READ FROM edecay.in
C END MG and MMM changed
...
C          ----------------------
C
C LAGPARM  = 0: chiral Lagrangian
C          = 1: SILH Lagrangian
C          = 2: Composite Higgs Lagrangian MCHM4 or MCHM5
C
C IELW = 0 no elw corrections in the SILH or MCHM4/5 parametrisation
C      = 1 w/ elw corrections in the SILH or MCHM4/5 parametrisation
C
C The coupling factors of the chiral Lagrangian are given by:
C
...
C CWW:     POINT-LIKE H-WW COUPLING
C CZZ:     POINT-LIKE H-ZZ COUPLING
C CWdW:    DERIVATIVE H-WW COUPLING
C CZdZ:    DERIVATIVE H-ZZ COUPLING
C
C The part of the effective chiral Lagrangian with the point-like 
C couplings reads:
C
C   L = [ Cgg/2 G^{a\mu\nu}G^a_{\mu\nu} 
C       + Cgaga/2 F^{\mu\nu}F_{\mu\nu}
...
C       + CZdZ Z_\nu \partial_\mu Z^{\mu\nu} ] h/v
C where G^{a\mu\nu}, F^{\mu\nu}, W^{+\mu\nu} AND Z^{\mu\nu} are the field 
C strength tensors of the gluon, photon, W and Z boson fields.
C
C The notation of the SILH Lagrangian parametrisation is:
C
C cHbar: \bar{c}_H vertex
C cTbar: \bar{c}_T vertex
C ctaubar, cmubar, ctbar, cbbar, ccbar, csbar: 
C the vertices \bar{c}_\psi
C with \psi = \tau, \mu, t, b, c, s.
C cWbar: \bar{c}_W vertex
C cBbar: \bar{c}_B vertex
C cHWbar: \bar{c}_{HW} vertex 
C cHBbar: \bar{c}_{HB} vertex
C cgambar: \bar{c}_\gamma vertex
C cgbar: \bar{c}_g vertex
C
C The various vertices are given in arXiv:XXXX
C
C The MCHM4 and MCHM5 parametrisation can be chosen by setting:
C
C FERMREPR = 1: MCHM4
C          = 2: MCHM5
C
C XI: value of xi = v^2/f^2, where v=246 GeV and f is the scale of the 
C     strong sector;
C     xi takes values between 0 (SM limit) and 1 (technicolor limit);
C     The H-VV (V=W,Z) couplings are modified compared to the SM by
C     \sqrt{1-\xi} both in MCHM4 and MCHM5;
C     the H-fermionfermion couplings are modified compared to the SM by
C     \sqrt{1-\xi} in MCHM4 (fermions in spinorial representation)
C     (1-2\xi)/\sqrt{1-\xi} in MCHM5 (fermions in fundamental represent.)
C
C =======================================================================
...
c -------------- common blocks needed in HDECAY subroutines ---------- c
     ...
     .                 ,GMSN1
      ...
      COMMON/FLAGS_HDEC/INDIDEC
c MG and MMM changed
c      COMMON/CPSM_HDEC/CPW,CPZ,CPTAU,CPMU,CPT,CPB,CPC,CPS,
c     .                 CPGAGA,CPGG,CPZGA,ICOUPELW
      COMMON/CPSM_HDEC/CPW,CPZ,CPTAU,CPMU,CPT,CPB,CPC,CPS,
     .                 CPGAGA,CPGG,CPZGA,CWWPAR,CZZPAR,CWPAR,CZPAR,
     .                 CWbarpar,ICOUPELW,ILAGPARAM,ICOUPVAR,ISM,IELWSILH
c      COMMON/CONTROLLFLAGS/ceffcflag
c end MG and MMM changed
...
c end MG and MMM changed
      ...
      READ(NI,*)
c MG and MMM changed 
      READ(NI,101)ILAGPARAM
      READ(NI,*)
      READ(NI,101)IELWSILH
      READ(NI,*)
c      READ(NI,101)ICOUPELW
c end MG and MMM changed
      READ(NI,100)CPW
      READ(NI,100)CPZ
      READ(NI,100)CPTAU
...
c MG and MMM added
      ...
      READ(NI,100)Cgbar
      READ(NI,*)
      READ(NI,101)IFERMREPR
      READ(NI,100)XI

      if(islhai.ne.0) then
         print*,''
         print*,'ATTENTION: You have chosen to read in parameters from a
     .n SLHA file. This is not possible if you want to use the effective
     .Lagrangian option. Setting default value islhai=0.'
         islhai = 0
      endif

      if(icoupvar.ne.1) then
         print*,''
         print*,'ATTENTION: You have chosen the SM coupling option. You 
     .will get the SM result for the decay widths, including elw and QCD
     . corrections. For the effective Lagrangian option you have to set 
     .icoupvar=1.'
      endif

      if(ihiggs.ne.0) then 
         print*,''
         print*,'ATTENTION: You have chosen the SUSY option. Taking defa
     .ult value ihiggs=0 for the effective Lagrangian option.'
         ihiggs=0
      endif

      if(ism4.ne.0) then 
         print*,''
         print*,'ATTENTION: You have chosen the fourth generation option
     .. Taking default value ism4=0 for the effective Lagrangian option.
     .'
         ism4=0
      endif

      if(ILAGPARAM.eq.1.and.icoupvar.ne.0) then
         print*,''
         print*,'ATTENTION: Do not choose the SILH coupling parameters t
     .oo large.'
      endif

c maggie changed 18/6/2013
c      if(icoupvar.eq.1.and.ilagparam.eq.1.or.ilagparam.eq.2) then
c         if(ielwsilh.eq.0) then
c            print*,''
c            print*,'Attention you use the SILH or MCHM4/5 parametrisatio
c     .n, but you have turned off the elw corrections. In order to turn t
c     .hem on, set ielw=1 in the input file.'
c         endif
c      endif

      if(icoupvar.eq.1.and.ilagparam.eq.1) then
         if(ielwsilh.eq.0) then
            print*,''
            print*,'Attention you use the SILH parametrisatiom, but you 
     .have turned off the elw corrections. In order to turn them on, set
     . ielw=1 in the input file.'
         endif
      endif
c end maggie changed 18/6/2013
c end MG and MMM added
...
c MG and MMM added
      ...
      IF(ICOUPVAR.NE.0.AND.ILAGPARAM.EQ.1) then
         SW2=1.D0-AMW**2/AMZ**2
         CW2=AMW**2/AMZ**2
         g2coup = 4.D0*dsqrt(2.D0)*GF*AMW**2
         pi = 4.D0*datan(1.D0)

         CPW    = 1.D0-cHbar/2.D0
         CPZ    = 1.D0-cHbar/2.D0-CTcapbar
         CPTAU  = 1.D0-(cHbar/2.D0+Ctaubar)
         CPMU   = 1.D0-(cHbar/2.D0+Cmubar)
         CPT    = 1.D0-(cHbar/2.D0+Ctbar)
         CPB    = 1.D0-(cHbar/2.D0+Cbbar)
         CPC    = 1.D0-(cHbar/2.D0+Ccbar)
         CPS    = 1.D0-(cHbar/2.D0+Csbar)
         CPGAGA = 8.D0*sw2*Cgambar
         alpha2 = dsqrt(2.D0)*Gf*amw**2/pi
c isospin coupling (g^2 = 4 pi alpha2)
         CPGG   = 8.D0/alpha2*Cgbar
c MG and MMM comment: We did not put the factor alphas here, hence we did 
c not write cpgg = 8*sw2*alphas/alpha*Cgbar = 8*alphas/alpha2*Cgbar. 
c The reason is that alphas depends 
c on the scale and takes different values in H->gluon gluon. We therefore 
c implemented this factor directly where the width H->gluon gluon is calculated.
         CPZGA  = (CHBbar-CHWbar-8.D0*cgambar*sw2)*
     .            dsqrt(sw2/cw2)
         CWWPAR = -2.D0*CHWbar
         CZZPAR = -2.D0*(cHWbar+cHBbar*sw2/cw2-4.D0*cgambar*sw2/cw2*sw2)
         CWPAR  = -2.D0*(CWbar+CHWbar)
         CZPAR  = -2.D0*(CWbar+CHWbar)-2.D0*(cbbbar+cHBbar)*sw2/cw2
         CWbarpar = CWbar

      ELSEIF(ICOUPVAR.NE.0.AND.ILAGPARAM.EQ.2) then
         if(xi.gt.1.D0) then
            print*,''
            print*,'Attention: xi out of range.'
            print*,''
         endif
         CPW = dsqrt(1.D0-xi)
         CPZ = dsqrt(1.D0-xi)
         if(IFERMREPR.eq.1) then 
            CPTAU = dsqrt(1.D0-xi)
            CPMU  = dsqrt(1.D0-xi)
            CPT   = dsqrt(1.D0-xi)
            CPB   = dsqrt(1.D0-xi)
            CPC   = dsqrt(1.D0-xi)
            CPS   = dsqrt(1.D0-xi)
         elseif(IFERMREPR.eq.2) then
            CPTAU = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
            CPMU  = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
            CPT   = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
            CPB   = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
            CPC   = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
            CPS   = (1.D0-2.D0*xi)/dsqrt(1.D0-xi)
         endif
         CPGAGA = 0.D0
         CPGG   = 0.D0
         CPZGA  = 0.D0
      ...
      smtmp = CPW*CPZ*CPTAU*CPMU*CPT*CPB*CPC*CPS

      smtmp = smtmp*(1.D0-CPGAGA)*(1.D0-CPGG)*(1.D0-CPZGA)*
     .     (1.D0-CWWPAR)*(1.D0-CZZPAR)*(1.D0-CWPAR)*(1.D0-CZPAR)

      if(smtmp.eq.1.D0.and.icoupvar.eq.1.and.ilagparam.eq.0) then
          print*,''
          print*,'You have chosen the parametrisation of the chiral Lagr
     .angian, and you are in the SM limit.'
          print*,''
          print*,'Note the chiral Lagrangian result in the SM limit diff
     .ers from the SM result as the chiral Lagrangian does not include e
     .lw corrections.'
          print*,''
      endif

c maggie added 18/6/2013
      if(smtmp.eq.1.D0.and.icoupvar.eq.1.and.ilagparam.eq.2) then
          print*,''
          print*,'You have chosen the parametrisation of the MCHM4/5 Lag
     .rangian, and you are in the SM limit.'
          print*,''
          print*,'Note the MCHM4/5 Lagrangian result in the SM limit dif
     .fers from the SM result as in this parametrisation elw corrections
     . are not included.'
          print*,''
      endif

      if(icoupvar.eq.1.and.ilagparam.eq.2) then
         print*,''
         print*,'You have chosen the parametrisation of the MCHM4/5 Lagr
     .angian. If the couplings you have chosen do not differ too much fr
     .om the corresponding SM couplings, we recommend to use the SILH pa
     .rametrisation (and translate your MCHM couplings accordingly to th
     .is parametrisation). In this way elw corrections are included as w
     .ell.'
      endif
...
C--CHECK 4TH GENERATION
     ...
     .  XXCP = 1
      IF(ISM4.NE.0)THEN
       ...
       CPS   = 1
      ENDIF
      IF(ISM4.NE.0.AND.XXCP.NE.0)THEN
       WRITE(6,*)'4TH GENERATION. TAKING DEFAULT COUPINGS = 1....'
      ENDIF
...
c MG and MMM changed
      WRITE(NPAR,20)'************** LAGRANGIAN 0 - chiral  1 - SILH  2 
     .- MCHM4/5 **************'
      WRITE(NPAR,8)'LAGPARAM = ',ILAGPARAM
c end MG and MMM changed
...
c MG and MMM added
      WRITE(NPAR,9)'CWWeff   = ',CWWPAR
      WRITE(NPAR,9)'CZZeff   = ',CZZPAR
      WRITE(NPAR,9)'CWeff    = ',CWPAR
      WRITE(NPAR,9)'CZeff    = ',CZPAR
      WRITE(NPAR,20)'**************************** SILH Lagrangian *******
     .************************'
      WRITE(NPAR,9)'CHbar    = ',chbar
      WRITE(NPAR,9)'CTbar    = ',ctcapbar
      WRITE(NPAR,9)'Ctaubar  = ',ctaubar
      WRITE(NPAR,9)'Cmubar   = ',cmubar
      WRITE(NPAR,9)'Ctbar    = ',ctbar
      WRITE(NPAR,9)'Cbbar    = ',cbbar
      WRITE(NPAR,9)'Ccbar    = ',ccbar
      WRITE(NPAR,9)'Csbar    = ',csbar
      WRITE(NPAR,9)'CWbar    = ',cwbar
      WRITE(NPAR,9)'CBbar    = ',cbbbar
      WRITE(NPAR,9)'CHWbar   = ',chwbar
      WRITE(NPAR,9)'CHBbar   = ',chbbar
      WRITE(NPAR,9)'Cgambar  = ',cgambar
      WRITE(NPAR,9)'Cgbar    = ',cgbar
      WRITE(NPAR,20)'******** MCHM4 (fermrepr=1), MCHM5 (fermrepr=2) par
     .ametrisation **********'
      WRITE(NPAR,8)'fermrepr = ',ifermrepr
      WRITE(NPAR,9)'xi       = ',xi
c end MG and MMM added
...
20    FORMAT(A74)
     ...
     .          NHB=91,NHC=92,NAA=93,NAB=94,NCA=95,NCB=96,NRA=97,NRB=98,
     ...
     .                 ,GMSN1
      ...
      IF(IHIGGS.EQ.0) THEN
c MG and MMM changed
       OPEN(NSA,FILE='br.eff1')
       OPEN(NSB,FILE='br.eff2')
c       OPEN(NSA,FILE='br.sm1')
c       OPEN(NSB,FILE='br.sm2')
c end MG and MMM changed
       IF(ISM4.NE.0)THEN
...
351    FORMAT('TAU1=',F8.3,1X,'TAU2=',F8.3,1X,'NL=',F8.3,1X,
     .        'EL=',F8.3,1X,'ER=',F8.3,1X,'NL1=',F8.3)
C
C
C **************************************************************
      ENDIF
...
c ----------------------------------- c
      ...
      write(nout,61) 2,'5.10 modif  # version number'
...
c MG and MMM changed
      complex*16 ampsm,dampsm,catsm,cabsm,cacsm,calsm,cawsm
c end MG and MMM changed
      COMMON/HMASS_HDEC/AMSM,AMA,AML,AMH,AMCH,AMAR
...
c     .                 CPGAGA,CPGG,CPZGA,ICOUPELW
     ...
     .                 CPGAGA,CPGG,CPZGA,CWWPAR,CZZPAR,CWPAR,CZPAR,
     .                 CWbarpar,ICOUPELW,ILAGPARAM,ICOUPVAR,ISM,IELWSILH
      COMMON/VVDECFLAGS/CVVPAR,CVPAR,ICPVVFLAG
c      COMMON/CONTROLLFLAGS/ceffcflag
c end MG and MMM changed
...
C  mass dependent NLO QCD corrections
       ...
       IF(ISM4.NE.0)THEN
        ...
        CATP = 2*CTTP*(1+(1-CTTP)*CF(CTTP))
        CABP = 2*CTBP*(1+(1-CTBP)*CF(CTBP))
       ENDIF
...
c end MMM 16/2/2014
        if(ilagparam.eq.0) then
           XFAC = CDABS(CAT+CAB+CAC+CATP+CABP)**2*FQCD
     .          + 4*DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)*
     .                    CPGG*4.D0*Pi/asg)*DQCD
     .          + (2*CPGG*4.D0*Pi/asg)**2*PQCD
C  mass dependent NLO QCD corrections
           XFAC0 =(CDABS(CAT)**2*(XHGG(1)+XHQQ(1)*NFEXT)
     .       + CDABS(CAB)**2*(XHGG(2)+XHQQ(2)*NFEXT)
     .       + 2*DREAL(DCONJG(CAT)*CAB)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
        elseif(ilagparam.eq.1) then
           ampsm = cat0+cab0+cac0
           dampsm = (cpt-1.D0)*cat0+(cpb-1.D0)*cab0+(cpc-1.D0)*cac0
           xfac = cdabs(ampsm)**2*fqcd 
     .          + 2.D0*dreal(dconjg(ampsm)*dampsm)*fqcd
     .          + 4.D0*dreal(dconjg(ampsm)*cpgg*4.D0*Pi)*dqcd
c mass dependent NLO corrections
           XFAC0 =
     .    ((1.D0+2.D0*(CPT-1.D0))*CDABS(CAT0)**2*(XHGG(1)+XHQQ(1)*NFEXT)
     .   + (1.D0-2.D0*(CPB-1.D0))*CDABS(CAB0)**2*(XHGG(2)+XHQQ(2)*NFEXT)
     .   + 2.D0*(1.D0+CPT-1.D0+CPB-1.D0)*
     .     (DCONJG(CAT0)*CAB0)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
        endif
c end MG and MMM changed
c =================================================================
c MMM 16/2/2014
c        if(iqcd.eq.0) then
c           print*,'NLO xfac,xfac0',xfac,xfac0,xfac0/xfac
c           xfac0 = 0.D0
c        endif
c        if(iqcdm.eq.0) then
c           print*,'M-NLO xfac,xfac0',xfac,xfac0,xfac0/xfac
c           xfac0 = 0.D0
c        endif
c end MMM 16/2/2014
       XFAC = XFAC + XFAC0
...
c mass dependent NLO corrections
     ...
     .     (DCONJG(CAT0)*CAB0)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
c           XFAC0 =
c     .      (CDABS(CAT0)**2*(XHGG(1)+XHQQ(1)*NFEXT)
c     .     + CDABS(CAB0)**2*(XHGG(2)+XHQQ(2)*NFEXT)
c     .   + 2.D0*DREAL(DCONJG(CAT0)*CAB0)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
        endif
c end MG and MMM changed
c =================================================================
       XFAC = XFAC + XFAC0
...
c MMM 16/2/2014
c        if(iqcd.eq.0) then
c           print*,'1-fqcd,dqcd,pqcd',fqcd,dqcd,pqcd
c           fqcd = 1.D0
c           dqcd = 1.D0
c           pqcd = 1.D0
c           print*,'0-fqcd,dqcd,pqcd',fqcd,dqcd,pqcd
c        endif
c end MMM 16/2/2014
c MG and MMM changed
        if(ilagparam.eq.0) then
...
C  mass dependent NLO QCD corrections
     ...
     .       + CDABS(CAB)**2*(XHGG(2)+XHQQ(2)*NFEXT)
     .       + 2*DREAL(DCONJG(CAT)*CAB)*(XHGG(3)+XHQQ(3)*NFEXT))*ASG/PI
c           print*,'xfac,xfac0,NNNLO',xfac,xfac0,xfac0/xfac
        elseif(ilagparam.eq.1) then
           ampsm = cat0+cab0+cac0
           dampsm = (cpt-1.D0)*cat0+(cpb-1.D0)*cab0+(cpc-1.D0)*cac0
           xfac = cdabs(ampsm)**2*fqcd 
...
c MMM 16/2/2014
c        if(iqcd.eq.0) then
c           print*,'NNNLO xfac,xfac0',xfac,xfac0,xfac0/xfac
c           xfac0=0.D0
c        endif
c        if(iqcdm.eq.0) then
c           print*,'M - NNNLO xfac,xfac0',xfac,xfac0,xfac0/xfac
c           xfac0=0.D0
c        endif
c end MMM 16/2/2014        
        XFAC = XFAC + XFAC0
...
cc        XFAC00 = CDABS(CAT0+CAB0+CAC0+CATP+CABP)**2*FQCD
c         XFAC00= DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)
c     .                     * (CAT0+CAB0+CAC0+CATP+CABP))*FQCD
c        ELSE
c         XFAC00 = CDABS(CAT+CAB+CAC+CATP+CABP)**2*FQCD
c     .          + 4*DREAL(DCONJG(CAT+CAB+CAC+CATP+CABP)*CPGG)*DQCD
c     .          + (2*CPGG)**2*PQCD
c        ENDIF
        ...
        if(ilagparam.eq.0) then
           XFAC00 = 0.D0
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              XFAC00 = CDABS(CAT0+CAB0+CAC0)**2*FQCD
           elseif(ielwsilh.eq.0) then
              XFAC00 = 0.D0
           endif
        endif
        if(ism.eq.1) then
...
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       ELSE
        IM = IGGELW
        FQCD0=HGGQCD(ASG,5)
        FAC4 = -ASG**2/PI**2*(77/288.D0*2 + (2/3.D0*5+19/8.D0)/3
     .       * (DLOG(AMBP**2/AMT**2)+DLOG(AMTP**2/AMT**2)))
        FQCD=HGGQCD2(ASG,5,AMH,AMT)+FAC4
        XFAC = CDABS(CAT+CAB+CAC+CATP+CABP)**2*FQCD
        HGG=HVV(AMH,0.D0)*(ASG/PI)**2*XFAC/8
        HGG = HGG * (1+GLGL_ELW4(IM,AMH))
...
c MG and MMM changed
         if(ilagparam.eq.0) then
            HMM=HFF(AMH,(AMMUON/AMH)**2)* CPMU**2
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HMM= HFF(AMH,(AMMUON/AMH)**2)*(1.D0+2.D0*(CPMU-1.D0))
     .             + HFF(AMH,(AMMUON/AMH)**2)*ELW(AMH,AMMUON,-1.D0,7.D0)
     .             + HFF(AMH,(AMMUON/AMH)**2)*(HFFSELF(AMH)-1.D0)
c     .         + HFF(AMH,(AMMUON/AMH)**2)*ELW(AMH,AMMUON,-1.D0,7.D0)*
c     .           (HFFSELF(AMH)-1.D0)
            elseif(ielwsilh.eq.0) then
               HMM= HFF(AMH,(AMMUON/AMH)**2)*(1.D0+2.D0*(CPMU-1.D0))
            endif
...
c      HMM=HFF(AMH,(AMMUON/AMH)**2)
c     .    *(1+ELW(AMH,AMMUON,-1.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    *SM4FACF
c     .    * CPMU**2
c      IF(ICOUPELW.EQ.0)THEN
c       HMM=HFF(AMH,(AMMUON/AMH)**2) * CPMU
c     .    * ((CPMU-1)
c     .       +(1+ELW(AMH,AMMUON,-1.D0,7.D0))
c     .       *HFFSELF(AMH))
c     .    * SM4FACF
...
C  H ---> TAU TAU
      ...
      ELSE
         if(ilagparam.eq.0) then
            HLL=HFF(AMH,(AMTAU/AMH)**2)* CPTAU**2
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HLL= HFF(AMH,(AMTAU/AMH)**2)*(1.D0+2.D0*(CPTAU-1.D0))
     .              + HFF(AMH,(AMTAU/AMH)**2)*ELW(AMH,AMTAU,-1.D0,7.D0)
     .              + HFF(AMH,(AMTAU/AMH)**2)*(HFFSELF(AMH)-1.D0)
c     .         + HFF(AMH,(AMTAU/AMH)**2)*ELW(AMH,AMTAU,-1.D0,7.D0)*
c     .           (HFFSELF(AMH)-1.D0)
            elseif(ielwsilh.eq.0) then
               HLL= HFF(AMH,(AMTAU/AMH)**2)*(1.D0+2.D0*(CPTAU-1.D0))
            endif
...
c      HLL=HFF(AMH,(AMTAU/AMH)**2)
c     .    *(1+ELW(AMH,AMTAU,-1.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    *SM4FACF
c     .    * CPTAU**2
c      IF(ICOUPELW.EQ.0)THEN
c       HLL=HFF(AMH,(AMTAU/AMH)**2) * CPTAU
c     .    * ((CPTAU-1)
c     .        +(1+ELW(AMH,AMTAU,-1.D0,7.D0))
c     .        *HFFSELF(AMH))
c     .    *SM4FACF
...
C  H --> SS
      ...
      ELSE
       ...
       ENDIF
       RATCOUP = CPT/CPS
       if(ilagparam.eq.1) RATCOUP = (CPS+CPT-1)/(2*CPS-1)
      ENDIF
     ...
     .           *QCDH(RMS**2/AMH**2)* CPS**2
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HS2= 3.D0*HFF(AMH,(RMS/AMH)**2)
     .              *QCDH(RMS**2/AMH**2)*(1.D0+2.D0*(CPS-1.D0))
     .              + 3.D0*HFF(AMH,(RMS/AMH)**2)
     .              *QCDH(RMS**2/AMH**2)*
     .              ELW(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD)
     .              + 3.D0*HFF(AMH,(RMS/AMH)**2)
     .              *QCDH(RMS**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .         + 3.D0*HFF(AMH,(RMS/AMH)**2)
c     .           *QCDH(RMS**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .           *ELW(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD) 
            elseif(ielwsilh.eq.0) then
               HS2= 3.D0*HFF(AMH,(RMS/AMH)**2)
     .              *QCDH(RMS**2/AMH**2)*(1.D0+2.D0*(CPS-1.D0))
            endif
...
c       HS2=3.D0*HFF(AMH,(RMS/AMH)**2)
c     .    *QCDH(RMS**2/AMH**2)
c     .    *(1+ELW(AMH,RMS,-1.D0/3.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    * CPS**2
c      IF(ICOUPELW.EQ.0)THEN
c       HS2=3.D0*HFF(AMH,(RMS/AMH)**2) * CPS
c     .    *QCDH(RMS**2/AMH**2)
c     .    * ((CPS-1)
c     .         +(1+ELW(AMH,RMS,-1.D0/3.D0,7.D0)*(1+XQCD))
c     .       *HFFSELF(AMH))
c      ENDIF
c end MG and MMM changed
...
c MG and MMM changed
     ...
     .    *TQCDH(AMS**2/AMH**2)*CPS**2
         elseif(ilagparam.eq.1) then 
            if(ielwsilh.eq.1) then
               HS1= 3.D0*HFF(AMH,(AMS/AMH)**2)
     .              *TQCDH(AMS**2/AMH**2)*(1.D0+2.D0*(CPS-1.D0))
     .              + 3.D0*HFF(AMH,(AMS/AMH)**2)
     .              *TQCDH(AMS**2/AMH**2)*ELW(AMH,RMS,-1.D0/3.D0,7.D0)
     .              + 3.D0*HFF(AMH,(AMS/AMH)**2)
     .              *TQCDH(AMS**2/AMH**2)*(HFFSELF(AMH)-1.D0) 
c     .           + 3.D0*HFF(AMH,(AMS/AMH)**2)
c     .    *TQCDH(AMS**2/AMH**2)*ELW(AMH,RMS,-1.D0/3.D0,7.D0)*
c     .           (HFFSELF(AMH)-1.D0)
            elseif(ielwsilh.eq.0) then
               HS1= 3.D0*HFF(AMH,(AMS/AMH)**2)
     .              *TQCDH(AMS**2/AMH**2)*(1.D0+2.D0*(CPS-1.D0))
...
c       HS1=3.D0*HFF(AMH,(AMS/AMH)**2)
c     .    *TQCDH(AMS**2/AMH**2)
c     .    *(1+ELW(AMH,RMS,-1.D0/3.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    *CPS**2
c      IF(ICOUPELW.EQ.0)THEN
c       HS1=3.D0*HFF(AMH,(AMS/AMH)**2) * CPS
c     .    *TQCDH(AMS**2/AMH**2)
c     .    * ((CPS-1)
c     .         +(1+ELW(AMH,RMS,-1.D0/3.D0,7.D0))
c     .       *HFFSELF(AMH))
...
C  H --> CC
      ...
      ELSE
       ...
       ENDIF
       RATCOUP = CPT/CPC
       if(ilagparam.eq.1) RATCOUP = (CPC+CPT-1)/(2*CPC-1)
      ENDIF
...
c MG and MMM changed 
     ...
     .           *QCDH(RMC**2/AMH**2)*CPC**2
     .           + DCC
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HC2=3.D0*HFF(AMH,(RMC/AMH)**2)
     .              *QCDH(RMC**2/AMH**2)*(1.D0+2.D0*(CPC-1.D0))
     .              +3.D0*HFF(AMH,(RMC/AMH)**2)
     .              *QCDH(RMC**2/AMH**2)*
     .              ELW(AMH,RMC,2.D0/3.D0,7.D0)*(1+XQCD)
     .              +3.D0*HFF(AMH,(RMC/AMH)**2)
     .              *QCDH(RMC**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .         +3.D0*HFF(AMH,(RMC/AMH)**2)
c     .           *QCDH(RMC**2/AMH**2)*ELW(AMH,RMC,2.D0/3.D0,7.D0)*
c     .           (HFFSELF(AMH)-1.D0)
     .              + DCC
            elseif(ielwsilh.eq.0) then
               HC2=3.D0*HFF(AMH,(RMC/AMH)**2)
...
c       HC2=3.D0*HFF(AMH,(RMC/AMH)**2)
c     .    *QCDH(RMC**2/AMH**2)
c     .    *(1+ELW(AMH,RMC,2.D0/3.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    *CPC**2
c     .   + DCC
c      IF(ICOUPELW.EQ.0)THEN
c       HC2=3.D0*HFF(AMH,(RMC/AMH)**2) * CPC
c     .    *QCDH(RMC**2/AMH**2)
c     .    * ((CPC-1)
c     .       +(1+ELW(AMH,RMC,2.D0/3.D0,7.D0)*(1+XQCD))
c     .       *HFFSELF(AMH))
c     .   + DCC
c      ENDIF
c end MG and MMM changed
...
c MG and MMM changed
     ...
     .    *TQCDH(AMC**2/AMH**2)*CPC**2
     .   + DCC
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HC1=3.D0*HFF(AMH,(AMC/AMH)**2)
     .              *TQCDH(AMC**2/AMH**2)*(1.D0+2.D0*(CPC-1.D0))
     .              +3.D0*HFF(AMH,(AMC/AMH)**2)
     .              *TQCDH(AMC**2/AMH**2)*ELW(AMH,AMC,2.D0/3.D0,7.D0)
     .              +3.D0*HFF(AMH,(AMC/AMH)**2)
     .              *TQCDH(AMC**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .         +3.D0*HFF(AMH,(AMC/AMH)**2)
c     .    *TQCDH(AMC**2/AMH**2)*ELW(AMH,AMC,2.D0/3.D0,7.D0)
c     .           *(HFFSELF(AMH)-1.D0)
     .              + DCC
            elseif(ielwsilh.eq.0) then
               HC1=3.D0*HFF(AMH,(AMC/AMH)**2)
     ...
     .              + DCC
         ...
         if(ism.eq.1) then
            HC1=3.D0*HFF(AMH,(AMC/AMH)**2)
     .           *TQCDH(AMC**2/AMH**2)
     .           *(1+ELW(AMH,AMC,2.D0/3.D0,7.D0))
     .           *HFFSELF(AMH)
     .           + DCC
         endif
c       HC1=3.D0*HFF(AMH,(AMC/AMH)**2)
c     .    *TQCDH(AMC**2/AMH**2)
c     .    *(1+ELW(AMH,AMC,2.D0/3.D0,7.D0))
c     .    *HFFSELF(AMH)
c     .    *CPC**2
c     .   + DCC
c      IF(ICOUPELW.EQ.0)THEN
c       HC1=3.D0*HFF(AMH,(AMC/AMH)**2) * CPC
c     .    *TQCDH(AMC**2/AMH**2)
c     .    * ((CPC-1)
c     .       +(1+ELW(AMH,AMC,2.D0/3.D0,7.D0))
c     .       *HFFSELF(AMH))
...
C  H --> BB :
      ...
      ELSE
       ...
       ENDIF
       RATCOUP = CPT/CPB
       if(ilagparam.eq.1) RATCOUP = (CPB+CPT-1)/(2*CPB-1)
      ENDIF
...
c MG and MMM changed
     ...
     .           *QCDH(RMB**2/AMH**2)* CPB**2
     .           + DBB
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HB2=3.D0*HFF(AMH,(RMB/AMH)**2)
     .              *QCDH(RMB**2/AMH**2)*(1.D0+2.D0*(CPB-1.D0))
     .              +3.D0*HFF(AMH,(RMB/AMH)**2)
     .              *QCDH(RMB**2/AMH**2)*
     .              ELW(AMH,RMB,-1.D0/3.D0,1.D0)*(1+XQCD)
     .              +3.D0*HFF(AMH,(RMB/AMH)**2)
     .              *QCDH(RMB**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .         +3.D0*HFF(AMH,(RMB/AMH)**2)
c     .           *QCDH(RMB**2/AMH**2)*ELW(AMH,RMB,-1.D0/3.D0,1.D0)*
c     .           (HFFSELF(AMH)-1.D0)
     .              + DBB 
            elseif(ielwsilh.eq.0) then
               HB2=3.D0*HFF(AMH,(RMB/AMH)**2)
...
c     .    *QCDH(RMB**2/AMH**2)
c     .    *(1+ELW(AMH,RMB,-1.D0/3.D0,1.D0))
c     .    *HFFSELF(AMH)
c     .    * CPB**2
c     .   + DBB
c       IF(ICOUPELW.EQ.0)THEN
c        HB2=3.D0*HFF(AMH,(RMB/AMH)**2) * CPB
c     .     *QCDH(RMB**2/AMH**2)
c     .     * ((CPB-1)
c     .        +(1+ELW(AMH,RMB,-1.D0/3.D0,1.D0)*(1+XQCD))
c     .        *HFFSELF(AMH))
c     .    + DBB
c       ENDIF
c end MG and MMM changed
...
c MG and MMM changed
     ...
     .           *TQCDH(AMB**2/AMH**2)* CPB**2
     .           + DBB
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HB1=3.D0*HFF(AMH,(AMB/AMH)**2)
     .              *TQCDH(AMB**2/AMH**2)*(1.D0+2.D0*(CPB-1.D0))
     .              +3.D0*HFF(AMH,(AMB/AMH)**2)
     .              *TQCDH(AMB**2/AMH**2)*ELW(AMH,AMB,-1.D0/3.D0,1.D0)
     .              +3.D0*HFF(AMH,(AMB/AMH)**2)
     .              *TQCDH(AMB**2/AMH**2)*(HFFSELF(AMH)-1.D0)
c     .         +3.D0*HFF(AMH,(AMB/AMH)**2)
c     .           *TQCDH(AMB**2/AMH**2)*ELW(AMH,AMB,-1.D0/3.D0,1.D0)*
c     .            (HFFSELF(AMH)-1.D0)
     .              + DBB
            elseif(ielwsilh.eq.0) then
               HB1=3.D0*HFF(AMH,(AMB/AMH)**2)
     ...
     .              + DBB
         ...
         if(ism.eq.1) then
            HB1=3.D0*HFF(AMH,(AMB/AMH)**2)
     .           *TQCDH(AMB**2/AMH**2)
     .           *(1+ELW(AMH,AMB,-1.D0/3.D0,1.D0))
     .           *HFFSELF(AMH)
     .           + DBB
         endif
c       HB1=3.D0*HFF(AMH,(AMB/AMH)**2)
c     .    *TQCDH(AMB**2/AMH**2)
c     .    *(1+ELW(AMH,AMB,-1.D0/3.D0,1.D0))
c     .    *HFFSELF(AMH)
c     .    * CPB**2
c     .   + DBB
c      IF(ICOUPELW.EQ.0)THEN
c       HB1=3.D0*HFF(AMH,(AMB/AMH)**2) * CPB
c     .    *TQCDH(AMB**2/AMH**2)
c     .    * ((CPB-1)
c     .        +(1+ELW(AMH,AMB,-1.D0/3.D0,1.D0))
c     .       *HFFSELF(AMH))
...
c MG and MMM changed
     ...
     .          *QCDH(XMT**2/XX(3)**2)* CPT**2
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              XY2= 3.D0*HFF(XX(3),(XMT/XX(3))**2)
     .             *QCDH(XMT**2/XX(3)**2)*(1.D0+2.D0*(CPT-1.D0))
     .             + 3.D0*HFF(XX(3),(XMT/XX(3))**2)
     .             *QCDH(XMT**2/XX(3)**2)*(HFFSELF(XX(3))-1.D0)
           elseif(ielwsilh.eq.0) then
              XY2= 3.D0*HFF(XX(3),(XMT/XX(3))**2)
     .             *QCDH(XMT**2/XX(3)**2)*(1.D0+2.D0*(CPT-1.D0))
           endif
...
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         XY2=3.D0*HFF(XX(3),(XMT/XX(3))**2) * CPT
c     .      *QCDH(XMT**2/XX(3)**2)
c     .      * ((CPT-1)
c     .         +HFFSELF(XX(3)))
c        ENDIF
...
c MG and MMM changed
     ...
     .          *TQCDH(AMT**2/XX(3)**2)* CPT**2
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              XY1=3.D0*HFF(XX(3),(AMT/XX(3))**2)
     .             *TQCDH(AMT**2/XX(3)**2)*(1.D0+2.D0*(CPT-1.D0))
     .             +3.D0*HFF(XX(3),(AMT/XX(3))**2)
     .             *TQCDH(AMT**2/XX(3)**2)*(HFFSELF(XX(3))-1.D0)
           elseif(ielwsilh.eq.0) then
              XY1=3.D0*HFF(XX(3),(AMT/XX(3))**2)
     .             *TQCDH(AMT**2/XX(3)**2)*(1.D0+2.D0*(CPT-1.D0))
           endif
...
c     .    *TQCDH(AMT**2/XX(3)**2)
c     .    *HFFSELF(XX(3))
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         XY1=3.D0*HFF(XX(3),(AMT/XX(3))**2) * CPT
c     .     *TQCDH(AMT**2/XX(3)**2)
c     .      * ((CPT-1)
c     .    *HFFSELF(XX(3)))
c        ENDIF
...
c MG and MMM changed
     ...
     .          *QCDH(XMT**2/XX(4)**2)* CPT**2
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              XY2=3.D0*HFF(XX(4),(XMT/XX(4))**2)
     .             *QCDH(XMT**2/XX(4)**2)*(1.D0+2.D0*(CPT-1.D0))
     .             +3.D0*HFF(XX(4),(XMT/XX(4))**2)
     .             *QCDH(XMT**2/XX(4)**2)*(HFFSELF(XX(4))-1.D0)
           elseif(ielwsilh.eq.0) then
              XY2=3.D0*HFF(XX(4),(XMT/XX(4))**2)
     .             *QCDH(XMT**2/XX(4)**2)*(1.D0+2.D0*(CPT-1.D0))
           endif
...
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         XY2=3.D0*HFF(XX(4),(XMT/XX(4))**2) * CPT
c     .      *QCDH(XMT**2/XX(4)**2)
c     .      * ((CPT-1)
c     .         +HFFSELF(XX(4)))
c        ENDIF
...
c MG and MMM changed
     ...
     .    *TQCDH(AMT**2/XX(4)**2)* CPT**2
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              XY1=3.D0*HFF(XX(4),(AMT/XX(4))**2)
     .             *TQCDH(AMT**2/XX(4)**2)*(1.D0+2.D0*(CPT-1.D0))
     .             +3.D0*HFF(XX(4),(AMT/XX(4))**2)
     .             *TQCDH(AMT**2/XX(4)**2)*(HFFSELF(XX(4))-1.D0)
           elseif(ielwsilh.eq.0) then
              XY1=3.D0*HFF(XX(4),(AMT/XX(4))**2)
     .             *TQCDH(AMT**2/XX(4)**2)*(1.D0+2.D0*(CPT-1.D0))
           endif
...
c     .    *TQCDH(AMT**2/XX(4)**2)
c     .    *HFFSELF(XX(4))
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         XY1=3.D0*HFF(XX(4),(AMT/XX(4))**2) * CPT
c     .      *TQCDH(AMT**2/XX(4)**2)
c     .      * ((CPT-1)
c     .         +HFFSELF(XX(4)))
c        ENDIF
...
c MG and MMM changed
     ...
     .            *QCDH(RMT**2/AMH**2)* CPT**2
         elseif(ilagparam.eq.1) then
            if(ielwsilh.eq.1) then
               HT2=3.D0*HFF(AMH,(RMT/AMH)**2)
     .              *QCDH(RMT**2/AMH**2)*(1.D0+2.D0*(CPT-1.D0))
     .              +3.D0*HFF(AMH,(RMT/AMH)**2)
     .              *QCDH(RMT**2/AMH**2)*(HFFSELF(AMH)-1.D0)
            elseif(ielwsilh.eq.0) then
               HT2=3.D0*HFF(AMH,(RMT/AMH)**2)
     .              *QCDH(RMT**2/AMH**2)*(1.D0+2.D0*(CPT-1.D0))
            endif
...
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         HT2=3.D0*HFF(AMH,(RMT/AMH)**2) * CPT
c     .      *QCDH(RMT**2/AMH**2)
c     .      * ((CPT-1)
c     .         +HFFSELF(AMH))
c        ENDIF
...
c MG and MMM changed
     ...
     .          *TQCDH(AMT**2/AMH**2)* CPT**2
        elseif(ilagparam.eq.1) then
           if(ielwsilh.eq.1) then
              HT1=3.D0*HFF(AMH,(AMT/AMH)**2)
     .             *TQCDH(AMT**2/AMH**2)*(1.D0+2.D0*(CPT-1.D0))
     .             +3.D0*HFF(AMH,(AMT/AMH)**2)
     .             *TQCDH(AMT**2/AMH**2)*(HFFSELF(AMH)-1.D0)
           elseif(ielwsilh.eq.0) then
              HT1=3.D0*HFF(AMH,(AMT/AMH)**2)
     .             *TQCDH(AMT**2/AMH**2)*(1.D0+2.D0*(CPT-1.D0))
           endif
...
c           HT1=3.D0*HFF(AMH,(AMT/AMH)**2)
c     .    *TQCDH(AMT**2/AMH**2)
c     .    *HFFSELF(AMH)
c     .    * CPT**2
c        IF(ICOUPELW.EQ.0)THEN
c         HT1=3.D0*HFF(AMH,(AMT/AMH)**2) * CPT
c     .      *TQCDH(AMT**2/AMH**2)
c     .      * ((CPT-1)
c     .         +HFFSELF(AMH))
c        ENDIF
...
C  H ---> GAMMA GAMMA
     ...
     .     * CFACQ_HDEC(0,AMH,XRMC)
       ...
       CAEP = 0
c MG and MMM changed
       CATSM = 4/3D0 * 2*CTT*(1+(1-CTT)*CF(CTT)) 
     .     * CFACQ_HDEC(0,AMH,XRMT)
       CABSM = 1/3D0 * 2*CTB*(1+(1-CTB)*CF(CTB)) 
     .     * CFACQ_HDEC(0,AMH,XRMB)
       CACSM = 4/3D0 * 2*CTC*(1+(1-CTC)*CF(CTC)) 
     .     * CFACQ_HDEC(0,AMH,XRMC)
       if(ilagparam.eq.0) then
        XFAC0= CDABS(CAT+CAB+CAC+CAL+CAW+CPGAGA*4.D0*Pi/ALPH)**2
       elseif(ilagparam.eq.1) then
          if(ielwsilh.eq.1) then
             ampsm  = CATSM+CABSM+CACSM+CAL00+CAW00
             dampsm = (CAT-CATSM)+(CAB-CABSM)+(CAC-CACSM)+
     .            (CAL-CAL00)+(CAW-CAW00)+2.D0*CWbarpar*CAW00

             xfac0 = cdabs(ampsm)**2
     .          +2.D0*dreal(dconjg(ampsm)*(dampsm+cpgaga*4.D0*Pi/ALPH))
     .        +CDABS(CAT00+CAB00+CAC00+CAL00+CAW00)**2*GAGA_ELW(AMT,AMH)
     ...
     .           +2.D0*dreal(dconjg(ampsm)*(dampsm+cpgaga*4.D0*Pi/ALPH))
       ...
       if(ism.eq.1) then
          xfac0 = CDABS(CAT00+CAB00+CAC00+CAL00+CAW00)**2*
     .         GAGA_ELW(AMT,AMH)
       endif
c       XFAC0= CDABS(CAT0+CAB0+CAC0+CAL+CAW+CPGAGA)**2*GAGA_ELW(AMT,AMH)
c       IF(ICOUPELW.EQ.0)THEN
cc       XFAC0= CDABS(CAT00+CAB00+CAC00+CAL00+CAW00)**2*GAGA_ELW(AMT,AMH)
c        XFAC0= DREAL(DCONJG(CAT0+CAB0+CAC0+CAL+CAW+CPGAGA)
c     .       * (CAT00+CAB00+CAC00+CAL00+CAW00))*GAGA_ELW(AMT,AMH)
c       ENDIF
c end MG and MMM changed.
       IF(ISM4.NE.0)THEN
        ...
        CATP0= 4/3D0 * 2*CTTP*(1+(1-CTTP)*CF(CTTP))
        CABP0= 1/3D0 * 2*CTBP*(1+(1-CTBP)*CF(CTBP))
        CATP = 4/3D0 * 2*CTTP*(1+(1-CTTP)*CF(CTTP))
     .       * CFACQ_HDEC(0,AMH,XRMTP)
        CABP = 1/3D0 * 2*CTBP*(1+(1-CTBP)*CF(CTBP))
     .       * CFACQ_HDEC(0,AMH,XRMBP)
        CAEP =         2*CTEP*(1+(1-CTEP)*CF(CTEP))
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c       CATP = 0
c       CABP = 0
c       CAEP = 0
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
     ...
     .       - 237*AMTP**2/10 - 117*AMBP**2/10 - 3*XXQ)
        ...
        XFAC0= CDABS(CAT0+CAB0+CAC0+CAL+CAW+CATP0+CABP0+CAEP)**2
     .       * ((1+CELW)**2 - 1)
...
c       XFAC = CDABS(CAT+CAB+CAC+CAL+CAW+CATP+CABP+CAEP+CPGAGA)**2

       HGA=HVV(AMH,0.D0)*(ALPH/PI)**2/16.D0*XFAC0
...
C  H ---> Z GAMMA
      ...
      ELSE
       ...
       Fcharm = -3*2D0/3*(1-4*2D0/3*SS)/DSQRT(SS*CS) * CPC
...
c      CTC = 4*XRMC**2/AMH**2*DCMPLX(1D0,-EPS)
       ...
       CAC = Fcharm*(CI1(CTC,CLC) - CI2(CTC,CLC))
     ...
     .     + ((1+2/CTW)*TS - (5+2/CTW))*CI1(CTW,CLW)) * CPW
       ...
       CAEP = 0
c MG and MMM changed
       FTSM = -3*2D0/3*(1-4*2D0/3*SS)/DSQRT(SS*CS) 
       FBSM = 3*1D0/3*(-1+4*1D0/3*SS)/DSQRT(SS*CS) 
       FLSM = (-1+4*SS)/DSQRT(SS*CS) 
c              CAT = FT*(CI1(CTT,CLT) - CI2(CTT,CLT))
       CATSM = FTSM*(CI1(CTT,CLT) - CI2(CTT,CLT))
       CABSM = FBSM*(CI1(CTB,CLB) - CI2(CTB,CLB))
       CACSM = FTSM*(CI1(CTC,CLC) - CI2(CTC,CLC))
       CALSM = FLSM*(CI1(CTL,CLE) - CI2(CTL,CLE))
       CAWSM = -1/DSQRT(TS)*(4*(3-TS)*CI2(CTW,CLW)
...
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
       IF(ISM4.NE.0)THEN
        ...
        CATP = FT*(CI1(CTTP,CLTP) - CI2(CTTP,CLTP))
        CABP = FB*(CI1(CTBP,CLBP) - CI2(CTBP,CLBP))
        CAEP = FL*(CI1(CTEP,CLEP) - CI2(CTEP,CLEP))
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
c       CATP = 0
c       CABP = 0
c       CAEP = 0
...
c       write(6,*)CDABS(CAW)**2,CDABS(CAT+CAB+CAW)**2
       ENDIF
c MG and MMM changed
       tmpalpha2 = dsqrt(2.D0)*Gf*amw**2/pi
       FPTLIKE = CPZGA*4.D0*Pi/dsqrt(ALPH*tmpalpha2*ss)
       if(ilagparam.eq.0) then
          XFAC = CDABS(CAT+CAB+CAC+CAL+CAW+FPTLIKE)**2
       elseif(ilagparam.eq.1) then
          ampsm  = CATSM+CABSM+CACSM+CALSM+CAWSM
          dampsm = (CAT-CATSM)+(CAB-CABSM)+(CAC-CACSM)
     .         +(CAL-CALSM)+(CAW-CAWSM)+2.D0*CWbarpar*CAWSM
          XFAC = CDABS(ampsm)**2
     .         +2.D0*dreal(dconjg(ampsm)*(dampsm+fptlike))
       endif
c end MG and MMM changed
...
c MG and MMM changed
       if(ilagparam.eq.0) then
          ...
          cvpar = cwpar
          ICPVVFLAG = 1
          CALL HTOVV_HDEC(0,AMH,AMW,GAMW,HTWWa)
          HWW = 3D0/2D0*GF*AMW**4/DSQRT(2D0)/PI/AMH**3*
     .         (HTWW0 * CPW**2 + HTWWa * CPW)
       elseif(ilagparam.eq.1) then
          if(ielwsilh.eq.1) then
             cvvpar = cwwpar
             cvpar = cwpar
             ...
             CALL HTOVV_HDEC(0,AMH,AMW,GAMW,HTWWa)
             HWW = 3D0/2D0*GF*AMW**4/DSQRT(2D0)/PI/AMH**3*
     .            (HTWW0*(1.D0+2.D0*(CPW-1.D0)) + HTWWa + (HTWW-HTWW0))
c for a check
c w/o elw corrections
c          HWW = 3D0/2D0*GF*AMW**4/DSQRT(2D0)/PI/AMH**3*
c     .         (HTWW0*(1.D0+2.D0*(CPW-1.D0)) + HTWWa)
c end for a check
          elseif(ielwsilh.eq.0) then
             cvvpar = cwwpar
             cvpar = cwpar
             ...
             CALL HTOVV_HDEC(0,AMH,AMW,GAMW,HTWWa)
             HWW = 3D0/2D0*GF*AMW**4/DSQRT(2D0)/PI/AMH**3*
     .            (HTWW0*(1.D0+2.D0*(CPW-1.D0)) + HTWWa)
          endif
       endif
       if(ism.eq.1) then
...
c end MG and MMM changed
     ...
     .       *HVVSELF(XX(4)) * CPW**2
       IF(ICOUPELW.EQ.0)THEN
        YY(3)=HVV(XX(3),AMW**2/XX(3)**2)
     .      * CPW * ((CPW-1) + HVVSELF(XX(3)))
        YY(4)=HVV(XX(4),AMW**2/XX(4)**2)
     .      * CPW * ((CPW-1) + HVVSELF(XX(4)))
       ENDIF
        HWW = FINT_HDEC(AMH,XX,YY)
       ELSE
        HWW=HVV(AMH,AMW**2/AMH**2)
...
C  H ---> Z Z
      IF(IONWZ.EQ.0)THEN
       IF(IFERMPHOB.NE.0.OR.ISM4.NE.0)THEN
...
c MG and MMM changed
       if(ilagparam.eq.0) then
          ...
          cvpar = czpar
          ICPVVFLAG = 1
          CALL HTOVV_HDEC(0,AMH,AMZ,GAMZ,HTZZa)
          HZZ = 3D0/4D0*GF*AMZ**4/DSQRT(2D0)/PI/AMH**3*
     .         (HTZZ0 * CPZ**2 + HTZZa * CPZ)
       elseif(ilagparam.eq.1) then
          if(ielwsilh.eq.1) then
             cvvpar = czzpar
             cvpar = czpar
             ...
             CALL HTOVV_HDEC(0,AMH,AMZ,GAMZ,HTZZa)
             HZZ = 3D0/4D0*GF*AMZ**4/DSQRT(2D0)/PI/AMH**3*
     .            (HTZZ0*(1.D0+2.D0*(CPZ-1.D0)) + HTZZa + (HTZZ-HTZZ0))
c for a check
c w/o elw corrections
c          HZZ = 3D0/4D0*GF*AMZ**4/DSQRT(2D0)/PI/AMH**3*
c     .         (HTZZ0*(1.D0+2.D0*(CPZ-1.D0)) + HTZZa)
c end check
          elseif(ielwsilh.eq.0) then
             cvvpar = czzpar
             cvpar = czpar
             ...
             CALL HTOVV_HDEC(0,AMH,AMZ,GAMZ,HTZZa)
             HZZ = 3D0/4D0*GF*AMZ**4/DSQRT(2D0)/PI/AMH**3*
     .            (HTZZ0*(1.D0+2.D0*(CPZ-1.D0)) + HTZZa)
          endif
       endif
       if(ism.eq.1) then
...
C  H --> BP BP :
       IF(AMH.LE.2*AMBP) THEN
        HBPBP = 0
       ELSE
        HBP2=3.D0*HFF(AMH,(RMBP/AMH)**2)
     .     *QCDH(RMBP**2/AMH**2)
     .     *HFFSELF(AMH)
        IF(HBP2.LT.0.D0) HBP2 = 0
        HBP1=3.D0*HFF(AMH,(AMBP/AMH)**2)
     .     *TQCDH(AMBP**2/AMH**2)
     .     *HFFSELF(AMH)
        RAT = 2*AMBP/AMH
...
C  H --> TP TP :
       IF(AMH.LE.2*AMTP) THEN
        HTPTP = 0
       ELSE
        HTP2=3.D0*HFF(AMH,(RMTP/AMH)**2)
     .     *QCDH(RMTP**2/AMH**2)
     .     *HFFSELF(AMH)
        IF(HTP2.LT.0.D0) HTP2 = 0
        HTP1=3.D0*HFF(AMH,(AMTP/AMH)**2)
     .     *TQCDH(AMTP**2/AMH**2)
     .     *HFFSELF(AMH)
        RAT = 2*AMTP/AMH
...
c      XFAC0= CDABS(CAT+CAB+CAC)**2
c      XFAC1= CDABS(CAT+CAB+CAC+CXB1+CXB2+CXT1+CXT2
c    .             +CXUL+CXUR+CXDL+CXDR)**2
c      write(6,*)'h -> gg: ',XFAC1/XFAC0
c      write(6,*)'h -> gg: ',CAT,CXT1,CXT2,
c    . CAB,CAC,CXB1,CXB2,CXUL,CXUR,CXDL,CXDR
...
C  H ---> TAU TAU
      ...
      ENDIF
c     write(6,*)'h: mu/tau = ',XGLM**2/XGLT**2
C  H --> SS
      XGLS = GLB
      XGHS = GHB
      XGAS = GAB
...
c      write(6,*)'h -> bb: ',COFSUSY_HDEC(I0,AMB,XMB,QQ)*AS0/PI,2*DGLB
       CALL BOTSUSY_HDEC(GLB,GHB,GAB,XGLB,XGHB,XGAB,QSUSY,LOOP)
      ENDIF
      RATCOUP = GLT/XGLB
...
c    .    *QCDH(RMB**2/AML**2)
c    .    *(1+ELW(AML,RMB,-1.D0/3.D0,1.D0))
c    .    *HFFSELF(AML)
c    .    + DBB
c      IF(HB2.LT.0.D0) HB2 = 0
...
c      write(6,*)'gamma gamma: ',AML,AMSL(1),AMSL(2),
c    .  CAT,CAB,CAC+CAL,CAW,CAH,CAX1+CAX2,
c    .  CXEL+CXER+CXUL+CXUR+CXDL+CXDR,CXB1+CXB2,CXT1+CXT2,CXL1+CXL2
       ELSE 
...
c     write(6,*)'h -> gamma gamma: ',AML,XFAC/XFACLO,HGA,HGA*XFACLO/XFAC
      XFACSM = CDABS(CAT/GLT+CAB/GLB+CAC/GLT+CAL/GLB+CAW/GLVV)**2
      XFAC0  = CDABS(CAT+CAB+CAC+CAL+CAW+CXL1)**2
c     write(6,*)'h -> ga ga: ',AML,XFAC0/XFACSM,HGA,HGA*XFACSM/XFAC0
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
C  H ---> Z GAMMA
      IF(AML.LE.AMZ)THEN
       HZGA=0
...
c      XFAC1= CDABS(CAT+CAB+CAC+CXB1+CXB2+CXT1+CXT2
c    .             +CXUL+CXUR+CXDL+CXDR)**2
c      write(6,*)'H -> gg: ',XFAC1/XFAC0,CAT,CXT1,CXT2,
c    . CAB,CAC,CXB1,CXB2,CXUL,CXUR,CXDL,CXDR
...
C  H ---> LL
      ...
      ENDIF
c     write(6,*)'H: mu/tau = ',XGHM**2/XGHT**2
C  H --> SS
      XGLS = GLB
      XGHS = GHB
      XGAS = GAB
...
502      AMA = (AMA0+AMA1)/2
     ...
     .     -DATAN(1.D0/DSQRT(4.D0*XA-1.D0)))/DSQRT(4.D0*XA-1.D0)
        XA2=3*GF**2/16.D0/PI**3*AMZ**4/AMH*GHAA**2*GAB**2*AMB**2
        HAA=XA1*XA2
c       write(6,*)'H -> AA:',AMH,AMA,1-XA,HAA
c       write(6,*)(XA-1)*(2-DLOG(XA)/2),
c    .     (1-5*XA)*(DATAN((2*XA-1)/DSQRT(4*XA-1))
c    .     -DATAN(1/DSQRT(4*XA-1)))/DSQRT(4*XA-1)
       ELSEIF (AMH.LE.XM2) THEN
        XX(1) = XM1-1D0
        XX(2) = XM1
        XX(3) = XM2
...
C  A ---> LL
      ...
      ENDIF
c     write(6,*)'A: mu/tau = ',XGAM**2/XGAT**2
C  A --> SS
      XGLS = GLB
      XGHS = GHB
      XGAS = GAB
...
c     Q0 = DSQRT(2*AMSQ**2+AMUR**2+AMDR**2)/2
      ...
      AD0 = AD + (CF*ALSP*AMGLU + 3*ALTP/2*AU + ALBP/4*AD)
     .         * DLOG(QB**2/Q0**2)
      AMDL0 = DSQRT(AMSQ**2 + (-CF*ALSP*AMGLU**2 + (ALTP*XT+ALBP*XB)/4)
     .                      * DLOG(QB**2/Q0**2))
      AMDR0 = DSQRT(AMDR**2 + (-CF*ALSP*AMGLU**2 + ALBP*XB/4)
     .                      * DLOG(QB**2/Q0**2))
      AU0 = AU + (CF*ALSP*AMGLU + ALTP/4*AU + 3*ALBP/2*AD)
     .         * DLOG(QT**2/Q0**2)
      AMUL0 = DSQRT(AMSQ**2 + (-CF*ALSP*AMGLU**2 + (ALTP*XT+ALBP*XB)/4)
     .                      * DLOG(QT**2/Q0**2))
      AMUR0 = DSQRT(AMUR**2 + (-CF*ALSP*AMGLU**2 + ALTP*XT/4)
     .                      * DLOG(QT**2/Q0**2))
c     write(6,*)
c     write(6,*)
...
c MG and MMM added
      COMMON/VVDECFLAGS/CVVPAR,CVPAR,ICPVVFLAG
      COMMON/PARAM_HDEC/GF,ALPH,AMTAU,AMMUON,AMZ,AMWW
c end MG and MMM added
     ...
     .                               /SP/SM/12)
c MG and MMM added
       if(ICPVVFLAG.eq.1) then
          pi = 4.D0*datan(1.D0)
          tmpcoupl = 2.D0*cvvpar/2.D0/amw**2
          tmpcoupl1 = cvpar/2.D0/amw**2

          gam = am2*LAMB(SP/AM2,SM/AM2)* (
     .         (-tmpcoupl*am2/2.D0*(1.D0-(sp+sm)/am2)
     .              -tmpcoupl1*(sp+sm))*2.D0*
     .         (1+LAMB(SP/AM2,SM/AM2)**2*AMH**4/SP/SM/12)
     .        +tmpcoupl*
     .         amh**4/12.D0/sp/sm*LAMB(SP/AM2,SM/AM2)**2*am2*
     .         (1.D0-(sp+sm)/am2)
     .         )
       endif
c end MG and MMM added
...
c      write(6,*)'Delta_b: ',DELTAMB,SBOT1,SBOT2,AMG,
c    .           2*ASH/3/PI*AMG*AMU*TANB*T_HDEC(SBOT1,SBOT2,AMG)
c>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
